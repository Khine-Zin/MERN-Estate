...
const CreateListing = () => {
  ...
  const navigate=useNavigate();
    ...
    const handleSubmit=async(e)=>{
      ...
      try{
        const response=await axios.post("http://localhost:3000/listing/create",data,{
            headers :{
    ...
    <main className='p-3 max-w-4xl mx-auto'>
        ...
        <form onSubmit={handleSubmit}  className='flex flex-col sm:flex-row gap-4'>
           ...
           <div  className='flex flex-col flex-1 gap-4'>
                ...
                <button disabled={loading || uploadLoading}  className='p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80'>{loading ? "Create Listing...." : "Create Listing"}</button>
