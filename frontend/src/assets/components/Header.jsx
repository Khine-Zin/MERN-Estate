...
useEffect(()=>{
 ...
 <h1 className='font-bold text-sm sm:text-xl flex flex-wrap'>
        <span className='text-slate-500'>X-mark</span>
    ...
    <ul className="flex gap-4">
        ...
        {user.currentUser?
        (<Link to="/profile"> <img className="rounded-full h-7 w-7 object-cover" src={user.currentUser.result.picture} /></Link>):
          (<Link to="/sign-in"><li className="hidden sm:inline text-slate-700 hover:underline font-semibold">Sign in</li></Link>)
        }
